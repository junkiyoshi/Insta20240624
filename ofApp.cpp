...
void ofApp::draw() {
	...
	vector<glm::vec3> base_location = {
		glm::vec3(-radius * 2.5, -radius * 2.5, 0), glm::vec3(0, -radius * 2.5, 0), glm::vec3(radius * 2.5, -radius * 2.5, 0),
		glm::vec3(-radius * 2.5, 0, 0), glm::vec3(0, 0, 0), glm::vec3(radius * 2.5, 0, 0),
		glm::vec3(-radius * 2.5, radius * 2.5, 0), glm::vec3(0, radius * 2.5, 0), glm::vec3(radius * 2.5, radius * 2.5, 0),
	};
	...
	for (int k = 0; k < base_location.size(); k++) {
		...
		color.setHsb(ofMap(k, 0, base_location.size(), 0, 255), 180, 255);
		...
		for (int i = 0; i < word.size(); i++) {
			...
			for (int outline_index = 0; outline_index < outline.size(); outline_index++) {
				...
				for (auto& vertex : vertices) {
					...
					auto angle_x = ofMap(ofNoise(noise_param.x, noise_location.x * 0.5, ofGetFrameNum() * 0.02), 0, 1, -PI * 0.25, PI * 0.25);
					auto angle_y = ofMap(ofNoise(noise_param.y, noise_location.y * 0.5, ofGetFrameNum() * 0.02), 0, 1, -PI * 0.25, PI * 0.25);
					auto angle_z = ofMap(ofNoise(noise_param.z, noise_location.z * 0.5, ofGetFrameNum() * 0.02), 0, 1, -PI * 0.25, PI * 0.25);
